...
//la unica diferencia entre templateStringHelperTranslate y templateStringHelperRun
//es que Runner hace lo de la secuencia de escape (ejemplo: '\n' lo reemplaza por un solo caracter(el salto de linea) )

//@CleanUp:
//Dos funciones practicamente identicas :/
...
//REGEX: (\`[^`]*\`)
export function templateStringHelperTranslate(source:string):TemplateString{
    ...
    let stringChunkBuffer = new String("");
...
//THIS FUNCTION DOES NOT WORK AT ALL IF source IS NOT ACCEPTED BY 
//REGEX: (\`[^`]*\`)
export function templateStringHelperRun(source:string):TemplateString{
    ...
    while(true){
        ...
        if(_char === '`'){
            ...
            //the following if is a chapuz para evitar que cerrar un template string con \` cause problemas
